...
const handleGetMonthlyStats = async (req, res) => {
  try {
    ...
    const [totalUsers, newUsersThisMonth, totalOrders, ordersThisMonth, inventoryData, yearlyOrders] = await Promise.all([
    ...
    // Create a complete month range for the year
    const monthNames = [
    ...
    yearlyOrders.forEach(({ _id, count }) => {
      ordersByMonth[_id].orders = count;
    });

    // Convert to array format for chart data
    const yearlyOrdersChart = Object.values(ordersByMonth);
